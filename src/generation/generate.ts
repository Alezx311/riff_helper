import { GenerationResult, StyleConfig, GenerateFor } from '../types';
import { TUNINGS, BASS_TUNING, STYLES, TOTAL_FRETS } from '../constants/music';
  ...
  for (let v = 0; v < 7; v++) {
    ...
    if (generateFor.guitar && guitarInput.length > 0) {
      if (isRemix) {
        ...
        genGuitar = generateContinuation(guitarInput, scaleNotes, genLen, strategy, styleConfig, guitarMidiFn, 6, state.key) || [];
    ...
    if (generateFor.bass && (bassInput.length > 0 || (guitarInput.length > 0 && genGuitar && genGuitar.length > 0))) {
      if (bassInput.length > 0) {
        if (isRemix) {
          genBass = generateRemix(bassInput, scaleNotes, Math.min(genLen, 6), strategy, styleConfig, getBassMidi, 4, state.key);
        } else {
          genBass = generateContinuation(bassInput, scaleNotes, Math.min(genLen, 6), strategy, styleConfig, getBassMidi, 4, state.key) || [];
    ...
    const fullGBeats = generateFor.guitar
      ? [...state.guitar.beats, ...gBeats]
      : [...state.guitar.beats];
    const fullBBeats = generateFor.bass
      ? [...state.bass.beats, ...bBeats]
      : [...state.bass.beats];
    ...
    results.push({
      ...
      drumPattern: generateFor.drums
        ? state.drums.pattern.map(r => [...r])
        : state.drums.pattern.map(r => [...r]),
